...
app.post("/api/create-checkout", async (req, res) => {
  const { amount, recipient, currency, senderEmail, feeRate } = req.body;
  ...
  try {
    ...
    await supabase.from("transactions").insert({ sender_name: senderEmail ? senderEmail.split("@")[0] : "Unknown", sender_email: senderEmail || null, recipient_name: recipient, amount: baseAmount, commission, total: amount, status: "pending", confirmation_code: confirmationCode, archived: false });
